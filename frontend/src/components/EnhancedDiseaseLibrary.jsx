...
import { Book, Languages, Globe, CheckCircle, AlertCircle, Loader2, Eye, EyeOff } from 'lucide-react';

const EnhancedDiseaseLibrary = () => {
  ...
  const translateDisease = async (diseaseName) => {
    try {
      ...
      if (result.success && result.data.successful > 0) {
        ...
        // Refresh diseases to get updated translations
        await fetchDiseases();
      } else {
  ...
  const translateAllDiseases = async () => {
    try {
      ...
      if (result.success) {
        // Update progress for each disease
        const newProgress = {};
...
export default EnhancedDiseaseLibrary;
